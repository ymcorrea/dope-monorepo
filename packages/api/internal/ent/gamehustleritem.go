...
func (*GameHustlerItem) scanValues(columns []string) ([]any, error) {
...
func (ghi *GameHustlerItem) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		default:
			ghi.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the GameHustlerItem.
// This includes values selected through modifiers, order, etc.
func (ghi *GameHustlerItem) Value(name string) (ent.Value, error) {
	return ghi.selectValues.Get(name)
}
...
func (ghi *GameHustlerItem) QueryHustler() *GameHustlerQuery {
	return NewGameHustlerItemClient(ghi.config).QueryHustler(ghi)
}
...
func (ghi *GameHustlerItem) Update() *GameHustlerItemUpdateOne {
	return NewGameHustlerItemClient(ghi.config).UpdateOne(ghi)
}
...
func (ghi *GameHustlerItem) Unwrap() *GameHustlerItem {
	_tx, ok := ghi.config.driver.(*txDriver)
	if !ok {
		panic("ent: GameHustlerItem is not a transactional entity")
	}
	ghi.config.driver = _tx.drv
	return ghi
}
