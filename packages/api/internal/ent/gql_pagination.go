...
func (c *AmountConnection) build(nodes []*Amount, pager *amountPager, after *Cursor, first *int, before *Cursor, last *int) {
	...
	c.Edges = make([]*AmountEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &AmountEdge{
			Node:   node,
...
func (p *amountPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		...
		if p.order.Field != DefaultAmountOrder.Field {
			b.Comma().Ident(DefaultAmountOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *bodypartPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultBodyPartOrder.Field {
			b.Comma().Ident(DefaultBodyPartOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *dopePager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultDopeOrder.Field {
			b.Comma().Ident(DefaultDopeOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *gamehustlerPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultGameHustlerOrder.Field {
			b.Comma().Ident(DefaultGameHustlerOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *gamehustleritemPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultGameHustlerItemOrder.Field {
			b.Comma().Ident(DefaultGameHustlerItemOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *gamehustlerquestPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultGameHustlerQuestOrder.Field {
			b.Comma().Ident(DefaultGameHustlerQuestOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *gamehustlerrelationPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultGameHustlerRelationOrder.Field {
			b.Comma().Ident(DefaultGameHustlerRelationOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *hustlerPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultHustlerOrder.Field {
			b.Comma().Ident(DefaultHustlerOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (c *ListingConnection) build(nodes []*Listing, pager *listingPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	c.Edges = make([]*ListingEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ListingEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func (p *listingPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultListingOrder.Field {
			b.Comma().Ident(DefaultListingOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *searchPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultSearchOrder.Field {
			b.Comma().Ident(DefaultSearchOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *walletPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultWalletOrder.Field {
			b.Comma().Ident(DefaultWalletOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (p *walletitemsPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.field).Pad().WriteString(string(direction))
		if p.order.Field != DefaultWalletItemsOrder.Field {
			b.Comma().Ident(DefaultWalletItemsOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
