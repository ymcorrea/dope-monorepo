...
func (hq *HustlerQuery) Order(o ...hustler.OrderOption) *HustlerQuery {
...
func (hq *HustlerQuery) Clone() *HustlerQuery {
	...
	return &HustlerQuery{
		config:        hq.config,
		ctx:           hq.ctx.Clone(),
		order:         append([]hustler.OrderOption{}, hq.order...),
...
func (hq *HustlerQuery) loadIndex(ctx context.Context, query *SearchQuery, nodes []*Hustler, init func(*Hustler), assign func(*Hustler, *Search)) error {
	...
	query.Where(predicate.Search(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(hustler.IndexColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "hustler_index" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
