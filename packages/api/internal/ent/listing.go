...
type ListingEdges struct {
	...
	namedInputs  map[string][]*Amount
	namedOutputs map[string][]*Amount
}
...
// NamedInputs returns the Inputs named value or an error if the edge was not
// loaded in eager-loading with this name.
func (l *Listing) NamedInputs(name string) ([]*Amount, error) {
	if l.Edges.namedInputs == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := l.Edges.namedInputs[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (l *Listing) appendNamedInputs(name string, edges ...*Amount) {
	if l.Edges.namedInputs == nil {
		l.Edges.namedInputs = make(map[string][]*Amount)
	}
	if len(edges) == 0 {
		l.Edges.namedInputs[name] = []*Amount{}
	} else {
		l.Edges.namedInputs[name] = append(l.Edges.namedInputs[name], edges...)
	}
}

// NamedOutputs returns the Outputs named value or an error if the edge was not
// loaded in eager-loading with this name.
func (l *Listing) NamedOutputs(name string) ([]*Amount, error) {
	if l.Edges.namedOutputs == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := l.Edges.namedOutputs[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (l *Listing) appendNamedOutputs(name string, edges ...*Amount) {
	if l.Edges.namedOutputs == nil {
		l.Edges.namedOutputs = make(map[string][]*Amount)
	}
	if len(edges) == 0 {
		l.Edges.namedOutputs[name] = []*Amount{}
	} else {
		l.Edges.namedOutputs[name] = append(l.Edges.namedOutputs[name], edges...)
	}
}
