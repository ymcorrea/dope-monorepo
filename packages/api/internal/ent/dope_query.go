...
type DopeQuery struct {
	...
	withNamedListings map[string]*ListingQuery
...
func (dq *DopeQuery) QueryLastSale() *ListingQuery {
	query := (&ListingClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DopeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Dope, error) {
	...
	if query := dq.withLastSale; query != nil {
		if err := dq.loadLastSale(ctx, query, nodes, nil,
			func(n *Dope, e *Listing) { n.Edges.LastSale = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withListings; query != nil {
		if err := dq.loadListings(ctx, query, nodes,
			func(n *Dope) { n.Edges.Listings = []*Listing{} },
			func(n *Dope, e *Listing) { n.Edges.Listings = append(n.Edges.Listings, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range dq.withNamedListings {
		if err := dq.loadListings(ctx, query, nodes,
			func(n *Dope) { n.appendNamedListings(name) },
			func(n *Dope, e *Listing) { n.appendNamedListings(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (dq *DopeQuery) loadLastSale(ctx context.Context, query *ListingQuery, nodes []*Dope, init func(*Dope), assign func(*Dope, *Listing)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Dope)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "listing_dope_lastsales" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dq *DopeQuery) loadListings(ctx context.Context, query *ListingQuery, nodes []*Dope, init func(*Dope), assign func(*Dope, *Listing)) error {
	...
	for _, n := range neighbors {
		fk := n.dope_listings
		if fk == nil {
			return fmt.Errorf(`foreign-key "dope_listings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "dope_listings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (dq *DopeQuery) loadIndex(ctx context.Context, query *SearchQuery, nodes []*Dope, init func(*Dope), assign func(*Dope, *Search)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "dope_index" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedListings tells the query-builder to eager-load the nodes that are connected to the "listings"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (dq *DopeQuery) WithNamedListings(name string, opts ...func(*ListingQuery)) *DopeQuery {
	query := (&ListingClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if dq.withNamedListings == nil {
		dq.withNamedListings = make(map[string]*ListingQuery)
	}
	dq.withNamedListings[name] = query
	return dq
}
