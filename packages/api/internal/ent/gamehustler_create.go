...
func (ghc *GameHustlerCreate) Save(ctx context.Context) (*GameHustler, error) {
	ghc.defaults()
	return withHooks[*GameHustler, GameHustlerMutation](ctx, ghc.sqlSave, ghc.mutation, ghc.hooks)
}
...
func (ghc *GameHustlerCreate) createSpec() (*GameHustler, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(gamehustler.Table, sqlgraph.NewFieldSpec(gamehustler.FieldID, field.TypeString))
	)
	...
	if value, ok := ghc.mutation.CreatedAt(); ok {
		_spec.SetField(gamehustler.FieldCreatedAt, field.TypeTime, value)
