...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Amount, c.BodyPart, c.Dope, c.Event, c.GameHustler, c.GameHustlerItem,
		c.GameHustlerQuest, c.GameHustlerRelation, c.Hustler, c.Item, c.Listing,
		c.Search, c.SyncState, c.Wallet, c.WalletItems,
	} {
		n.Use(hooks...)
	}
}
...
// A call to `Intercept(f, g, h)` equals to `amount.Intercept(f(g(h())))`.
func (c *AmountClient) Intercept(interceptors ...Interceptor) {
	c.inters.Amount = append(c.inters.Amount, interceptors...)
}

// Create returns a builder for creating a Amount entity.
...
func (c *AmountClient) Query() *AmountQuery {
	return &AmountQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAmount},
...
func (c *AmountClient) Interceptors() []Interceptor {
	return c.inters.Amount
}

func (c *AmountClient) mutate(ctx context.Context, m *AmountMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AmountCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AmountUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AmountUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AmountDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Amount mutation op: %q", m.Op())
	}
}
...
func (c *ListingClient) QueryInputs(l *Listing) *AmountQuery {
	query := (&AmountClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := l.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(listing.Table, listing.FieldID, id),
...
type (
	hooks struct {
		Amount, BodyPart, Dope, Event, GameHustler, GameHustlerItem, GameHustlerQuest,
		GameHustlerRelation, Hustler, Item, Listing, Search, SyncState, Wallet,
