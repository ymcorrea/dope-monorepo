...
func (bpc *BodyPartCreate) Save(ctx context.Context) (*BodyPart, error) {
	return withHooks[*BodyPart, BodyPartMutation](ctx, bpc.sqlSave, bpc.mutation, bpc.hooks)
}
...
func (bpc *BodyPartCreate) createSpec() (*BodyPart, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(bodypart.Table, sqlgraph.NewFieldSpec(bodypart.FieldID, field.TypeString))
	)
	...
	if value, ok := bpc.mutation.GetType(); ok {
		_spec.SetField(bodypart.FieldType, field.TypeEnum, value)
		_node.Type = value
	}
	if value, ok := bpc.mutation.Sex(); ok {
		_spec.SetField(bodypart.FieldSex, field.TypeEnum, value)
		_node.Sex = value
	}
	if value, ok := bpc.mutation.Rle(); ok {
		_spec.SetField(bodypart.FieldRle, field.TypeString, value)
		_node.Rle = value
	}
