	...
	for j := range columns {
		...
		case item.FieldBestAskPriceEth:
			if value, ok := values[j].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field best_ask_price_eth", values[j])
			} else if value.Valid {
				i.BestAskPriceEth = value.Float64
			}
...
func (i *Item) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", i.BestAskPriceEth))
	builder.WriteByte(')')
