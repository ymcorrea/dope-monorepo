...
func (lc *ListingCreate) Save(ctx context.Context) (*Listing, error) {
	return withHooks[*Listing, ListingMutation](ctx, lc.sqlSave, lc.mutation, lc.hooks)
}
...
func (lc *ListingCreate) createSpec() (*Listing, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(listing.Table, sqlgraph.NewFieldSpec(listing.FieldID, field.TypeString))
	)
	...
	if value, ok := lc.mutation.Source(); ok {
		_spec.SetField(listing.FieldSource, field.TypeEnum, value)
		_node.Source = value
	}
