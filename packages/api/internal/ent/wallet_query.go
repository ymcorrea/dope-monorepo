...
func (wq *WalletQuery) loadDopes(ctx context.Context, query *DopeQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *Dope)) error {
	...
	query.Where(predicate.Dope(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(wallet.DopesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "wallet_dopes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (wq *WalletQuery) loadItems(ctx context.Context, query *WalletItemsQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *WalletItems)) error {
	...
	query.Where(predicate.WalletItems(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(wallet.ItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "wallet_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (wq *WalletQuery) loadHustlers(ctx context.Context, query *HustlerQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *Hustler)) error {
	...
	query.Where(predicate.Hustler(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(wallet.HustlersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "wallet_hustlers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
